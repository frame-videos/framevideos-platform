...
// .wrangler/tmp/bundle-BoiJvq/checked-fetch.js
...
// .wrangler/tmp/bundle-BoiJvq/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-BoiJvq/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
storage.post("/upload-url", authenticate2, async (c) => {
  try {
    const user = c.get("user");
    const { videoId, contentType, expiresIn } = await c.req.json();
    if (!videoId) {
    ...
    return c.json({ error: "Failed to generate upload URL", message: error.message }, 500);
  }
});
storage.post("/upload", authenticate2, async (c) => {
  try {
    const user = c.get("user");
    ...
    if (!file || !videoId || !title) {
      return c.json({ error: "file, videoId, and title are required" }, 400);
    }
    const storageService = new MockStorageService();
    ...
    return c.json({ error: "Failed to upload video", message: error.message }, 500);
  }
});
storage.post("/thumbnail/:videoId", authenticate2, async (c) => {
  try {
    const user = c.get("user");
    ...
    const storageService = new MockStorageService();
    const buffer = await file.arrayBuffer();
    const uploadResult = await storageService.uploadThumbnail(
      user.tenantId,
      videoId,
      buffer,
      {
    ...
    return c.json({ error: "Failed to upload thumbnail", message: error.message }, 500);
  }
});
storage.get("/signed-url/:videoId", authenticate2, async (c) => {
  try {
    const user = c.get("user");
    const videoId = c.req.param("videoId");
    ...
    return c.json({ error: "Failed to generate signed URL", message: error.message }, 500);
  }
});
storage.get("/download/:videoId", authenticate2, async (c) => {
  try {
    const user = c.get("user");
    const videoId = c.req.param("videoId");
    const video = await db.getVideoById(videoId);
    ...
    const storageService = new MockStorageService();
    const videoObject = await storageService.getVideo(user.tenantId, videoId);
    if (!videoObject) {
    ...
    return c.json({ error: "Failed to download video", message: error.message }, 500);
  }
});
storage.get("/stream/:videoId", authenticate2, async (c) => {
  try {
    const user = c.get("user");
    const videoId = c.req.param("videoId");
    const video = await db.getVideoById(videoId);
    ...
    if (range) {
      ...
      if (!rangeObject) {
        return c.json({ error: "Failed to get video range" }, 500);
      }
    ...
    return c.json({ error: "Failed to stream video", message: error.message }, 500);
  }
});
storage.delete("/:videoId", authenticate2, async (c) => {
  try {
    const user = c.get("user");
    const videoId = c.req.param("videoId");
    const video = await db.getVideoById(videoId);
    ...
    const storageService = new MockStorageService();
    await storageService.deleteVideo(user.tenantId, videoId);
