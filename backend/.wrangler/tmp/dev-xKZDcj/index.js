...
// .wrangler/tmp/bundle-2drULX/checked-fetch.js
...
var init_auth = __esm({
  "src/auth.ts"() {
    ...
    init_checked_fetch();
    init_modules_watch_stub();
    JWT_SECRET = "frame-videos-secret-key-change-in-production-12345";
...
// .wrangler/tmp/bundle-2drULX/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-2drULX/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var Database = class {
  ...
  async getVideosByTenant(tenantId) {
    ...
    for (const video of this.videos.values()) {
      if (video.tenantId === tenantId) videos2.push(video);
    }
    return videos2.sort(
...
videos.get("/", authenticate, async (c) => {
  ...
  const tenantVideos = await db.getVideosByTenant(user.tenantId);
  return c.json({
    videos: tenantVideos,
...
tenants.post("/", async (c) => {
  try {
    ...
    const existingTenant = await db.getTenantByDomain(domain);
    if (existingTenant) {
...
tenants.get("/domain/:domain", async (c) => {
...
app.route("/api/v1/videos", videos_default);
