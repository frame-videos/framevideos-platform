...
export default function VideoPage() {
  ...
  const loadVideo = async () => {
    try {
      ...
      const likeRes = await fetch(`/api/v1/videos/${videoId}/like`);
      if (likeRes.ok) {
  ...
  const loadRelatedVideos = async () => {
    try {
      const res = await fetch(`/api/v1/videos?limit=5&exclude=${videoId}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleLike = async () => {
    try {
      ...
      const res = await fetch(`/api/v1/videos/${videoId}/like`, { method });
